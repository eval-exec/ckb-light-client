...
pub(crate) struct BlocksProofRequest {
    ...
    /// The height each requested block hash was matched at by `BlockFilters`.
    ///
    /// `GetBlocksProof` carries only hashes, and (as of CKB v0.203.0) the wire
    /// message has no field to carry the expected height, so we keep the mapping
    /// on our side and check the proved header's number against it.
    expected_heights: HashMap<H256, BlockNumber>,
    when_sent: u64,
...
impl BlocksProofRequest {
    ...
    /// The height we matched `block_hash` at, if it was part of this request.
    pub(crate) fn expected_height(&self, block_hash: &H256) -> Option<BlockNumber> {
        self.expected_heights.get(block_hash).copied()
    }
...
impl Peers {
    ...
    pub fn add_matched_blocks(
        ...
        block_hashes: Vec<(Byte32, BlockNumber, bool)>,
        filter_source: Option<PeerIndex>,
    ) {
        for (block_hash, block_number, proved) in block_hashes {
